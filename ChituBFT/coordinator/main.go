...
func main() {
	...
	flag.IntVar(&batch, "b", 30000, "batch size")
	flag.IntVar(&payload, "p", 600, "payload")
	flag.IntVar(&test_time, "t", 5, "test time")
	flag.IntVar(&interval, "i", 1, "client reqs size")
	flag.Parse()
	fmt.Printf("batch: %+v, payload: %+v, test time: %+v, client reqs size: %+v\n", batch, payload, test_time, interval)
	var dep common.Devp
	fileName2 := "../test/devip.json"
...
func (c *Coordinator) Finish(req *common.CoorStatistics, resp *common.Response) error {
	...
	fmt.Printf("node %+v execution states: %+v \n", req.ID, req.States)
